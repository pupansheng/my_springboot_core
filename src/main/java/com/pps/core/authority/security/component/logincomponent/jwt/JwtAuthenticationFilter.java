package com.pps.core.authority.security.component.logincomponent.jwt;
...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse, FilterChain filterChain) throws ServletException, IOException {
        ...
        if(isNeedJwt){
            log.debug("网络请求：{}  已经授权",requestURL);
        ...
        if(isNeedAutu){
            log.debug("网络请求：{}  为不受限制网址",requestURL);
            filterChain.doFilter(httpServletRequest, httpServletResponse);
            return;
        }
        ...
        if(!isCanJwt){
            log.debug("网络请求：{}  不在jwt允许范围内 采用其他方式验证",requestURL);
            filterChain.doFilter(httpServletRequest, httpServletResponse);
            return;
        }

        if(jwtUtil.isEnable()){
            log.debug("已开启jwt登录------------> token登录filter处理：");
            String header = httpServletRequest.getHeader(jwtUtil.getHeaderParam());
            logger.debug("请求token:"+header);
